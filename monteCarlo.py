...
1. import image
2. get image canvas size
3. iterate in interval of random points to find area (N)
4. generate random (x, y) pairs (N pair)
5. check if its color is black or white
6. store black ones in a array
7. apply monte carlo equation
8. print result

1 pixel (X) = 0.0264583333 centimeter
"""
...
def show_random_points(image, x, y):
    ...
    fig, ax = plt.subplots()
    ax.imshow(image)
    ax.set_title('PLOT OF 1000 RANDOM POINTS ON THE IMAGE\nClose this window to continue')
